...
Goldbach :: Goldbach(int64_t value) {
  this -> count = 0;
  this -> value = value;
  this -> isEven = true;
...
void Goldbach :: validateEven() {
  if (this -> value %2 != 0)
    this -> isEven = false;
}

void Goldbach :: validateNegative() {
  if (this -> value >= 0)
...
bool Goldbach :: validatePrime(int number) {
  bool answer = true;
  if (number > 1) {
...
void Goldbach :: generatePrimeNumbers() {
...
void Goldbach :: generateGoldbachSums() {
  this -> isEven ? generateStrongSums() : generateWeakSums();
}

void Goldbach :: generateStrongSums() {
...
void Goldbach :: generateWeakSums() {
  std::vector<int64_t> sums;
  std::vector<int64_t> primeNumbers = this -> primeNumbers;
  int value = this -> value;
  int count = primeNumbers.size();
...
void Goldbach :: generateCount() {
  this -> count = this -> isEven ? this -> sums.size() / 2
                                  : this -> sums.size() / 3;
}

std::vector<std::string> Goldbach :: getSolutions() {
